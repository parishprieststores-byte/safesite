...
// Works with the app's own "Install App" button (#btn-install), or adds one if it is missing.
(function () {
  'use strict';
  ...
  function showHelp() {
    ...
    wrap.style.cssText = 'position:fixed;inset:0;z-index:100002;background:rgba(0,0,0,.55);display:flex;' +
  ...
  function hideInstall() {
    installEls().forEach(function (el) { el.style.display = 'none'; el.hidden = true; });
    if (floatBtn && floatBtn.parentNode) floatBtn.parentNode.removeChild(floatBtn);
    floatBtn = null;
  }

  function showInstall() {
    installEls().forEach(function (el) { el.hidden = false; el.style.display = ''; });
  }
