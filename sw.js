...
// After you upload a new index.html, change v2 to v3 (and so on) so phones refresh their saved copy.
const CACHE_VERSION = 'safesite-v2';
const CORE = [
  './',
  './index.html',
  './manifest.json',
  ...
  return new Promise((resolve) => {
    ...
    fetch(req)
      .then((res) => {
        clearTimeout(timer);
        if (res && res.ok) {
          caches.open(CACHE_VERSION).then((c) => c.put('./index.html', res.clone())).catch(() => {});
        }
        finish(res);
      })
      .catch(() => {
        clearTimeout(timer);
        fromCache().then((r) => {
          if (!r) { settled = true; resolve(Response.error()); } else { finish(r); }
        });
      });
  });
}
